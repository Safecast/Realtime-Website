 ...
 * modified by Rob Oudendijk for Air
 */


/* Safecast API URL format */
define('API_URI_INFO_FORMAT', 'http://dev.safecast.org/devices/%s.json');
define('API_URI_MAXIMUM_FORMAT', 'http://dev.safecast.org/en-US/measurements.json?device_id=%s&unit=cpm&per_page=1&order=value+desc');
define('API_URI_LAST_FORMAT', 'http://dev.safecast.org/en-US/measurements.json?device_id=%s&unit=cpm&per_page=1&order=captured_at+desc');
/* added by rob for Air */
define('API_URI_MAXIMUM_FORMAT_PM25', 'http://dev.safecast.org/en-US/measurements.json?device_id=%s&unit=pm2.5&per_page=1&order=value+desc');
define('API_URI_LAST_FORMAT_PM25', 'http://dev.safecast.org/en-US/measurements.json?device_id=%s&unit=pm2.5&per_page=1&order=captured_at+desc');
/* end added by rob for air */
...
function updateSensorMeasurement($postId, $uri, $measurementType) {
    ...
    /* added by rob for Air */
    if (strpos($sensorType, "PM2.5") !== false || strpos($sensorType, "PM25") !== false) {
        $pm25 = (float)$measurement->value;
        if (VERBOSE) {
            printf("> PM2.5 = %s\n", $pm25);
        }
    ...
    update_post_meta($postId, $measurementType.'_pm25', $pm25);
            ...
            if ($sensorId) {
                try {
                    ...
                    if (getSensorType(get_the_ID(), $sensorId)) {
                        ...
                        updateSensorMeasurement(get_the_ID(),
                            sprintf(API_URI_LAST_FORMAT_PM25, $sensorId),
                            'sensor_measurement_last_pm25');

                        updateSensorMeasurement(get_the_ID(),
                            sprintf(API_URI_MAXIMUM_FORMAT_PM25, $sensorId),
                            'sensor_measurement_max_pm25');
                    }
                } catch (\Exception $e) {
